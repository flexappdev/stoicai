...
- [x] PBI-2.1 `scripts/enrich.ts` + `scripts/enrich-loop.sh` — Claude batch enricher with lenient `coerceEnrichment` (drops invalid themes instead of failing batches). DB-mode runs in 1000-item passes (Supabase REST cap) via the loop wrapper. **Live state: 900 / 4,332 items enriched, 335 at quality≥85, 5 at quality≥95.** Remaining 3,432 paused on OpenRouter wallet (task #24); script is idempotent — `bash scripts/enrich-loop.sh` resumes when wallet refills.
